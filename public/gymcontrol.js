...
async function verAluno(id) {
  ...
  const sexoStr = (a.sexo===1||a.sexo==='Masculino') ? 'Masculino'
                : (a.sexo===2||a.sexo==='Feminino')  ? 'Feminino' : 'Outro';
  ...
  if (grid) {
    if (!turmas.length) { grid.innerHTML = '<div style="color:var(--gray-500);font-size:13px;padding:8px">Nenhuma turma cadastrada.</div>'; }
      ...
      grid.innerHTML = turmas.map(t => {
        ...
        const badgeOcup = t.ocupacao_pct >= 80 ? '<span class="badge badge-warning">Quase cheia</span>' : '<span class="badge badge-success">Ativa</span>';
        return `
          <div class="card">
...
async function salvarFicha() {
  ...
  /* Envia todos os exercícios preenchidos */
  for (let n = 1; n <= _numExercicios; n++) {
...
async function carregarAgenda() {
  ...
  if (tbody) {
    if (!lista.length) { _vazio('lista-agenda', 9, 'Nenhuma aula agendada. Clique em <strong>➕ Nova Aula</strong> para começar.'); }
    else {
  ...
  if (grid) {
    if (!lista.length) { grid.innerHTML = '<div style="color:var(--gray-500);font-size:13px;padding:8px">Nenhuma aula agendada.</div>'; }
    else {
